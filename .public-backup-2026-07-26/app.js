import { TOOLS, TOOL_ORDER, TOOLS_WITH_CUSTOM_FLOW } from './templates.js';
...
function route() {
  ...
  window.scrollTo(0, 0);
  if (view === 'tool' && TOOLS[arg]) {
    if (TOOLS_WITH_CUSTOM_FLOW && TOOLS_WITH_CUSTOM_FLOW.has(arg)) return renderDampLogger();
    return renderTool(arg);
  }
...
function renderTool(id) {
  ...
  app.innerHTML = `
    <div class="wrap">
      ...
      <div class="panel fade-up d2">
        ...
        <p id="groundErr" style="display:none;color:#c0392b;font-size:.88rem;margin-top:4px">Please select your reason above before continuing.</p>
        <p class="disclaimer">${DISCLAIMER}</p>
  ...
  app.querySelectorAll('.ground').forEach(node => {
    node.addEventListener('click', () => {
      ...
      const err = document.getElementById('groundErr');
...
function onGenerate(id) {
  ...
  draft.values = values;
  if (missing) {
    const el = document.getElementById('f_' + t.fields.find(f => f.required && !values[f.id])?.id);
    if (el) { el.focus(); el.style.outline = '2px solid #c0392b'; el.addEventListener('input', () => { el.style.outline = ''; }, { once: true }); }
    return;
  }
  if (!draft.ground) {
    const err = document.getElementById('groundErr');
    if (err) { err.style.display = 'block'; document.querySelector('.grounds').scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }
    return;
  }
  const err = document.getElementById('groundErr');
  if (err) err.style.display = 'none';
