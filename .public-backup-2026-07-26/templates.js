...
{{yourAddress}}` }
  ...
  delay_repay: {
    id: 'delay_repay',
    label: 'Train Delay Repay',
    tagline: 'Claim compensation for a delayed train',
    blurb: 'Delayed 30+ minutes? Fill in your journey details and download a pre-filled compensation claim form to submit to your train operator.',
    fields: [
      { id: 'yourName',    label: 'Your full name',         type: 'text',     required: true },
      { id: 'yourAddress', label: 'Your address',           type: 'textarea', required: true },
      { id: 'yourEmail',   label: 'Your email address',     type: 'text',     required: true },
      { id: 'journeyDate', label: 'Date of delayed journey',type: 'date',     required: true },
      { id: 'operator',    label: 'Train operator',         type: 'select',   required: true,
        options: ['Avanti West Coast','c2c','Chiltern Railways','CrossCountry','East Midlands Railway',
                  'Govia Thameslink Railway (Thameslink, Southern, Great Northern, Gatwick Express)',
                  'Grand Central','Greater Anglia','Great Western Railway (GWR)',
                  'Hull Trains','Island Line','LNER (London North Eastern Railway)',
                  'London Overground','London Northwestern Railway','Lumo',
                  'Merseyrail','Northern','ScotRail','South Western Railway',
                  'Southeastern','TfL Rail / Elizabeth line','Transpennine Express',
                  'Transport for Wales','West Midlands Trains','Other'] },
      { id: 'fromStation', label: 'Departure station',      type: 'text',     required: true },
      { id: 'toStation',   label: 'Destination station',    type: 'text',     required: true },
      { id: 'scheduledDep', label: 'Scheduled departure time', type: 'text', required: true, placeholder: 'e.g. 09:15' },
      { id: 'actualArr',   label: 'Actual arrival time',    type: 'text',     required: true, placeholder: 'e.g. 10:52' },
      { id: 'delayMins',   label: 'Delay at destination (minutes)', type: 'text', required: true },
      { id: 'ticketType',  label: 'Ticket type',            type: 'select',   required: true,
        options: ['Anytime Single','Anytime Return','Off-Peak Single','Off-Peak Return',
                  'Super Off-Peak','Advance Single','Season Ticket','Railcard discounted','Other'] },
      { id: 'ticketCost',  label: 'Ticket cost (£)',        type: 'text',     required: true },
      { id: 'ticketRef',   label: 'Booking reference or ticket number (optional)', type: 'text', required: false },
      { id: 'detail',      label: 'Any additional details (optional)', type: 'textarea', required: false }
    ],
    grounds: [
      { id: 'delay30',
        label: '30–59 minute delay — 25% of single fare (or 50% of return)',
        hint: 'Most operators pay 25% of a single or 50% of a return leg for a 30–59 min delay.',
        body: `Dear Delay Repay Team,

RE: DELAY REPAY CLAIM
...
From:             {{fromStation}}
To:               {{toStation}}
Scheduled depart: {{scheduledDep}}
Actual arrival:   {{actualArr}}
Delay:            {{delayMins}} minutes
Ticket type:      {{ticketType}}
Ticket cost:      £{{ticketCost}}
Booking reference: {{ticketRef}}

I am writing to claim compensation under your Delay Repay scheme for the above journey, which arrived {{delayMins}} minutes late.

For a delay of 30–59 minutes, I am entitled to 25% of the single fare (or 50% of the cost of the relevant leg of a return ticket) under your National Rail Conditions of Travel obligations. {{detail}}

I attach / will upload proof of travel (ticket, booking confirmation or e-ticket) and evidence of the delay (for example, a National Rail Journey Enquiries delay check for {{journeyDate}}).

Please process my claim and pay the appropriate amount to my nominated account or as a rail travel voucher.

DISCLAIMER: This claim form was prepared using AppealMate, a self-help tool. AppealMate is not a law firm and does not provide legal advice. I am submitting this claim myself. I am not assigning this claim to any third party, and AppealMate has no involvement in the submission or outcome of this claim.
...
{{yourEmail}}` },
      ...
      { id: 'delay60',
        label: '60–119 minute delay — 50% of single fare',
        hint: 'A delay of 1–2 hours typically qualifies for 50% of the single or return leg fare.',
        body: `Dear Delay Repay Team,
...
For a delay of 60–119 minutes I am entitled to 50% of the single fare (or the cost of the relevant leg of a return ticket) under the National Rail Conditions of Travel. {{detail}}

I attach / will upload proof of travel (ticket or booking confirmation) and evidence of the delay.

Please process my claim and pay the appropriate amount to my nominated account or as a rail travel voucher.

DISCLAIMER: This claim form was prepared using AppealMate, a self-help tool. AppealMate is not a law firm and does not provide legal advice. I am submitting this claim myself and I am not assigning this claim to any third party.
...
{{yourEmail}}` },
      ...
      { id: 'delay120',
        label: '120+ minute delay — full fare refund',
        hint: 'A delay of 2 hours or more entitles you to a full refund of the affected single or return leg.',
        body: `Dear Delay Repay Team,

RE: DELAY REPAY CLAIM — FULL REFUND
...
I am writing to claim a full refund under your Delay Repay scheme for the above journey, which arrived {{delayMins}} minutes late.

A delay of 120 minutes or more entitles me to 100% of the single fare (or the full cost of the relevant return leg) under the National Rail Conditions of Travel. {{detail}}

I attach / will upload proof of travel and evidence of the delay.

Please process my claim and arrange a full refund to my nominated account or as a rail travel voucher.
...
{{yourEmail}}` },
      ...
      { id: 'season',
        label: 'Season ticket — daily pro-rata compensation',
        hint: 'Season ticket holders are entitled to compensation calculated on a per-journey basis.',
        body: `Dear Delay Repay Team,

RE: DELAY REPAY CLAIM — SEASON TICKET
...
Actual arrival:   {{actualArr}}
Delay:            {{delayMins}} minutes
Ticket type:      Season Ticket
Season ticket cost: £{{ticketCost}}
Booking reference: {{ticketRef}}

I am writing to claim compensation under your Delay Repay scheme for the above journey made using my season ticket, which arrived {{delayMins}} minutes late.

Compensation for season ticket holders is calculated as a proportion of the daily cost of the season ticket. I ask you to calculate and apply the appropriate amount based on my season ticket. {{detail}}

I attach / will upload evidence of my season ticket and evidence of the delay.

Please process my claim and credit the appropriate amount to my nominated account or by extension of my season ticket.
...
// damp_mould uses a custom multi-step flow (renderDampLogger) in app.js, not the
// generic renderTool flow. It appears in TOOLS so the home card and "My letters"
// label lookups work, but its fields/grounds are handled separately.
export const DAMP_TOOL_ID = 'damp_mould';
export const TOOLS_WITH_CUSTOM_FLOW = new Set(['damp_mould']);

export const TOOL_ORDER = ['parking', 'escalate', 'deposit', 'delay_repay', 'delay', 'parcel', 'counciltax', 'energy', 'holiday', 'lba', 'damp_mould'];

// Minimal stub so home card + "My letters" have a label — flow is in app.js
TOOLS.damp_mould = {
  id: 'damp_mould',
  label: 'Damp & Mould Logger',
  tagline: 'Evidence log + landlord notice (Awaab\'s Law)',
  blurb: 'Log damp and mould with timestamped photos, then download an evidence PDF and a formal landlord notice under Awaab\'s Law 2025.',
  fields: [],  // handled by custom flow
  grounds: []  // handled by custom flow
};
